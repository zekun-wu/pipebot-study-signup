...
export function buildIcs({ startIso, durationMin, mode, uid }) {
  ...
  const location =
    mode === "in_person"
      ? IN_PERSON_ADDRESS
      : teamsLink || "Microsoft Teams (link will follow)";
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    ...
    `DESCRIPTION:${
      mode === "in_person" || !teamsLink
        ? "Your session for the AI Agents & Workflows user study."
        : `Your session for the AI Agents & Workflows user study. Join via Microsoft Teams: ${teamsLink}`
    }`,
    "END:VEVENT",
    "END:VCALENDAR",
  ];
  ...
  const modeBlock =
    mode === "in_person"
      ? `<p><strong>How to take part — in person 🏛️</strong><br/>
         ...
         <p><strong>📝 Note:</strong> if you have to cancel or reschedule, please drop an email to
         <a href="mailto:${CONTACT_EMAIL}">${CONTACT_EMAIL}</a>.</p>`
      : `<p><strong>📝 Note:</strong> please make sure you join the link below
         ...
         ${
           teamsLink
             ? `<p style="text-align:center;margin:18px 0;">
                ...
                <p style="font-size:13px;color:#6b7280;">Or copy this link:
                  <a href="${teamsLink}">${teamsLink}</a>
                </p>`
